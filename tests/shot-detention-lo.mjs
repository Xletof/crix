// EVIDENCE — DETENTION'S EMERGENCY-POWER COMPOSITION, before and after.
//
//   node tests/shot-detention-lo.mjs detention-lo-before
//   node tests/shot-detention-lo.mjs detention-lo-after
//
...
const TAG = process.argv[2] || 'detention-lo-after';
...
const OPEN = [
  ['centre',   800,  700],   // dead centre of the escort floor — the complaint
  ['gate',    1240,  700],   // the processing checkpoint, at the east clamp
  ['cells',    800,  598],   // the north containment bank, at the north clamp
  ['west',     420,  700],   // the walk's western half
];
...
for (const [name, px, py] of OPEN) {
  ...
  const w = await atStation(px, py);
  // dx is a constant -48 here: `midPoint` is measured against the camera's
  // full rect while the game viewport is inset under the HUD bar. What this
  // watches for is DRIFT away from that constant, and the clamps.
  console.log('     cam', JSON.stringify(w));
  await pause(true); await shot(`dark-${name}`); await pause(false);
}

// A PAN ALONG THE TRANSFER FLOOR. Six frames at 160px steps down the walk's
// own axis. A reflection that shimmers or crawls does it BETWEEN frames — a
// single station cannot show it.
console.log('— pan along the transfer floor —');
for (let i = 0; i < 6; i++) {
  await station(360 + i * 176, 700); await setDark(true);
...
await place(800, 700); await page.waitForTimeout(400); await setDark(true);
...
for (const [name, px, py] of [['centre', 800, 700], ['gate', 1240, 700], ['cells', 800, 598]]) {
  await station(px, py);
  const wn = await atStation(px, py);
  console.log('     cam', JSON.stringify(wn));
  await pause(true); await shot(`normal-${name}`); await pause(false);
}

// ── LATE VADER. The hierarchy question: the environment must stay under the
//    combat. Forced moves use the registry's own lowercase ids and every cast
//    is asserted to have ENTERED — a refused cast reads exactly like a failed
//    one, and the player must be alive for the cast to be accepted at all.
...
if (!haveBoss) console.error('  !! no boss — the Vader frames below are missing');
  ...
  await setDark(true);
  await place(800, 760, 800, 560);
  await page.waitForTimeout(300); await settle(); await setDark(true);
  await pause(true); await shot('dark-vader'); await pause(false);

  for (const id of ['saberthrow', 'sabercombo']) {
    await hush(); await place(800, 820, 800, 540);
    await page.waitForTimeout(250); await settle();
    if (!(await cast(id))) continue;
    await page.waitForTimeout(360); await setDark(true);
    await pause(true); await shot(`dark-${id}`); await pause(false);
  }

  await hush(); await place(800, 780, 800, 580);
  await page.waitForTimeout(250); await settle();
  await page.evaluate(() => {
    const gs = window.game.scene.getScene('Game');
