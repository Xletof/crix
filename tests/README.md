...
| `smoke-junction.mjs` | **assertion test, third arena.** In `run-all`. The reactor junction's structural truths — 11 wall bodies (8 cover + 3 solid props), the two wall control panels carrying no body, the emissive layer outside `roomLayer` and at the light depth with **zero faces**, power determinism through an outage and a mid-outage teardown, and a five-room lifecycle that hands the layer back and forth with BOTH other styled arenas. Plus three groups the other two rooms do not have: five of eight cover objects must take the unpowered tint (the dark state depends on that ratio), `stripEvery`/`accentEvery` must be 0 and no floor mark may be a `ring` or `pad` (the baseline's nine orange-red bars and two telegraph-shaped rings), and the room must be NEITHER approved arena — not their perimeter styles, not their landmark props, not their faces, and carrying `conduit` rather than the chamber's `trench` or the hangar's `track`. It also freezes the hangar, the chamber and detention as literals. A/B'd against `0af4cf9`: it fails there with 15 findings. |
